...
function parseRaidViewEmbed(embed) {
  ...
  for (const line of partyField.value.split('\n')) {
    ...
    if (fatigueMillis > 0) {
      // Try Discord mention first (<@ID>), fall back to display name (@name)
      const mentionMatch = line.match(/<@(\d+)>/);
      // Extract display name: everything after @ up to common suffixes like [MVP], | text, - DIGITS DMG, or Fatigued
      const nameMatch = line.match(/@(.+?)(?:\s*\[|\s*\||\s*-\s*[\d,]+\s*DMG|\s+Fatigued)/);
      fatiguedEntries.push({
        userId: mentionMatch ? mentionMatch[1] : null,
  ...
  return fatiguedEntries.length ? fatiguedEntries : null;
}
